...
if __name__ == "__main__":
    ...
    os.mkdir(os.path.join(temp, "tmp"))
    os.mkdir(os.path.join(temp, "output"))
    args = [
        "--training_data=%s/part-*" % taxi_data,
        "--output_dir=%s/output" % temp,
        "--schema_file=%s/chicago_taxi_schema.pbtxt" % taxi_data,
        "--temp_location=%s/tmp" % temp,
        "--runner=DirectRunner"
    ]

    Preprocess.run(args=args)
    shutil.rmtree(temp)
